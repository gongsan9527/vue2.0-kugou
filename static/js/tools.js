...
export default {
	...
	hanlder (){
		...
		this.bigDot.on('touchend',function(event){
			let nowCurrentTime = _this.nowX / _this.bigBarContainer.width() * _this.durationTime;
			_this.audioEle.currentTime = nowCurrentTime;

			//拖动歌词矫正
			for(let i = 0,len = _this.lyricList.length;i<len;i++){	
				if(nowCurrentTime.toFixed(2) > _this.lyricList[i][0]) {
					_this.targetNumber = i - 1;
				};
			};
			_this.audioEle.play();
		});
